...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    <cac:AllowanceCharge>
        ...
        <cbc:Amount currencyID="{{ $document->currency_type_id }}">{{ $document->generalApplyNumberFormat($charge->amount) }}</cbc:Amount>
        <cbc:BaseAmount currencyID="{{ $document->currency_type_id }}">{{ $document->generalApplyNumberFormat($charge->base) }}</cbc:BaseAmount>
    </cac:AllowanceCharge>
    @endforeach
    @endif
